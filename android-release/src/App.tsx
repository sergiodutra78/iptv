...
const Inicio = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      const url = getActivePlaylistUrl();
      if (!url) return;
      setLoading(true);
      try {
    ...
    return (
      <div className="flex flex-col h-screen overflow-hidden bg-black text-white">
        <div className="flex items-center gap-3 px-6 py-4 border-b border-zinc-800">
          <Logo size="sm" />
          <div>
            <p className="text-sm font-black tracking-tighter italic">Configuración inicial</p>
            <p className="text-xs text-zinc-500">Ingresá tus credenciales IPTV para comenzar</p>
          </div>
        </div>
