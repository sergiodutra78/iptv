...
export const IPTV_CONFIG = {
    ...
    xtreamCodes: {
        baseUrl: "http://latinchannel.tv:8080",
...
export const getActivePlaylistUrl = (): string | null => {
    ...
    if (stored) {
        try {
            ...
            // Autocorrección de typo previo del usuario en el celular (latinchannell con doble L)
            if (configToUse.xtreamCodes?.baseUrl?.includes('latinchannell')) {
                localStorage.removeItem('iptv_config');
                configToUse = IPTV_CONFIG; // Forzar el hardcoded correcto
            }
        } catch (e) {
