...
const Series = () => {
    ...
    useEffect(() => {
        if (!selectedSeries) { setMetadata(null); return; }
        MetadataService.getSeriesMetadata(selectedSeries.name).then(setMetadata);
    }, [selectedSeries]);

    const filteredSeries = useMemo(() => {
        let result = series;
        if (selectedCategory !== 'Todos') result = result.filter(s => s.group === selectedCategory);
        if (searchQuery) {
    ...
    const displaySeries = useMemo(() => filteredSeries.slice(0, visibleCount), [filteredSeries, visibleCount]);
                    ...
                    <div className="relative h-full flex items-end p-6 gap-6 max-w-7xl mx-auto w-full">
                        ...
                        <div className="flex-1 pb-4 flex flex-col justify-end">
                            ...
                            <div className="flex flex-wrap items-center gap-2 mb-3">
                                ...
                                {metadata?.year && <span className="px-1.5 py-0.5 bg-zinc-900 text-zinc-300 text-xs font-bold rounded border border-zinc-800">{metadata.year}</span>}
                                <span className="px-1.5 py-0.5 bg-primary/20 text-primary text-xs font-bold rounded border border-primary/20 uppercase">{selectedSeries.group}</span>
                                    ...
                                    >
                                        <div className="w-10 h-10 rounded-lg bg-zinc-800 flex items-center justify-center flex-shrink-0 group-hover:bg-primary transition-colors">
                                            {watched ? (
                                                ...
                                                <PlayCircle size={20} className="text-zinc-500 group-hover:text-white" />
                                            )}
                                        </div>
                                        <div className="flex-1 min-w-0">
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="p-5 pb-3">
                ...
                {/* Categorías en scroll horizontal (mobile-friendly) */}
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-5">
                {loading ? (
                    <div className="flex flex-col items-center justify-center h-full gap-4">
                        ...
                        <p className="text-zinc-500 font-medium">Cargando series...</p>
                    </div>
                        ...
                        {viewMode === 'grid' ? (
                            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-3">
                                {displaySeries.map(item => (
                                    <MovieCard key={item.id} movie={item} onClick={() => setSelectedSeries(item)} />
                                ))}
                            </div>
                        ) : (
                            <div className="flex flex-col gap-3">
                                {displaySeries.map(item => (
                                    <SeriesListItem key={item.id} item={item} onClick={() => setSelectedSeries(item)} />
                                ))}
                            </div>
                        )}
