...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ url, title, subtitle, type = 'live', onClose, onNext, onPrev, onToggleChannelList, onToggleEPG }) => {
    ...
    useEffect(() => {
        ...
        setCurrentPlayingUrl(streamUrl); // Guardar para debug

        // --- NATIVE FALLBACK FOR ANDROID ---
        ...
        if (plugins && plugins.streamingMedia) {
            ...
            return; // Evita que hls.js intente cargarlo en el DOM
        }
        ...
        >
            <video
                ...
                onError={() => {
                    ...
                    setErrorDetails(`HTML5 Video Error: Code ${err?.code || 'X'} - ${err?.message || "Format not supported"} | URL: ${currentPlayingUrl.substring(0, 40)}...`);
                }}
            />
