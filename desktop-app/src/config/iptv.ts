...
export const getActivePlaylistUrl = (): string | null => {
    ...
    if (stored) {
        try {
            ...
            // Si el objeto existe pero tiene campos vacíos, usar IPTV_CONFIG como fallback
            if (parsed.xtreamCodes) {
                parsed.xtreamCodes.baseUrl = parsed.xtreamCodes.baseUrl || IPTV_CONFIG.xtreamCodes.baseUrl;
                parsed.xtreamCodes.username = parsed.xtreamCodes.username || IPTV_CONFIG.xtreamCodes.username;
                parsed.xtreamCodes.password = parsed.xtreamCodes.password || IPTV_CONFIG.xtreamCodes.password;
            } else {
                parsed.xtreamCodes = IPTV_CONFIG.xtreamCodes;
            }
            configToUse = parsed;
        } catch (e) {
